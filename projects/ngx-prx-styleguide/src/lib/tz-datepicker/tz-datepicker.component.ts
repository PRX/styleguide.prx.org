...
export class TzDatepickerComponent implements OnInit {
  ...
  get date() { return this.model && this.model.finalDate ? this.model.finalDate : this._date }
  ...
  ngOnInit() {
    this.timezones = this.tzDataSvc.fetchTzs().pipe(
      map((response) => {
  ...
  tzDateModelInit() {
    ...
    const timeString = this.supportsTimeInput ?
